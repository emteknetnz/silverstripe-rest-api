...
If the endpoint `ACCESS` is set to anything except `PUBLIC` then an `x-csrf-token` header needs to be sent with the request. A valid token is generated by `SecurityToken::getSecurityID()`. Within the CMS is easily available to javascript from `window.ss.config.SecurityID;`.
